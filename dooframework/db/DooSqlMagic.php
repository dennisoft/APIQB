...
class DooSqlMagic {
    ...
    public function connect() {
        ...
        try {
            if ($this->dbconfig[4] == 'sqlite') {
                ...
                if (isset($this->dbconfig[7])) {
                    $this->pdo = new PDO("{$this->dbconfig[4]}:host={$this->dbconfig[0]};dbname={$this->dbconfig[1]};port={$this->dbconfig[7]}", $this->dbconfig[2], $this->dbconfig[3], array(PDO::ATTR_PERSISTENT => $this->dbconfig[5]));
                } else {
    ...
    public function relate($model, $rmodel, $opt = null) {
        ...
        foreach ($repeated_vars as $r) {
            ...
            if (!in_array($r, $defined_class_vars)
            )
                continue;
